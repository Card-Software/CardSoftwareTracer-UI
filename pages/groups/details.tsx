...
import { organizationManagementProxy } from '@/proxies/organization-management.proxy';
import { userProxy } from '@/proxies/user.proxy';
import Select from 'react-select'; // Importamos el componente de react-select
...
const GroupDetails = () => {
  ...
  const [group, setGroup] = useState<Group>({
    ...
    description: '',
    ownerRef: '',
    membersEmail: [], // Emails seleccionados
  });
  const [isLoading, setIsLoading] = useState(false);
      ...
      if (isEditMode && id) {
        ...
        try {
          ...
          if (groupToEdit) {
            ...
            setSelectedOrganization(groupToEdit.ownerRef || '');
            setSelectedUsers(groupToEdit.membersEmail.map(email => ({ label: email, value: email })) || []);
          }
        } catch (error) {
          console.error('Error fetching group:', error);
        } finally {
  ...
  // Función para manejar la selección de correos electrónicos
  const handleEmailChange = (selectedOptions: any) => {
    setSelectedUsers(selectedOptions);
    const emails = selectedOptions.map((option: { value: string }) => option.value);
    setGroup((prev) => ({
      ...prev,
      membersEmail: emails, // Guardamos los correos electrónicos seleccionados
    }));
  };

  // Función para enviar el formulario (creación/edición)
  const handleSubmit = async () => {
    setIsLoading(true);
    try {
      if (isEditMode) {
        ...
        const createdGroup = await userAuthorizationProxy.createGroup({
          ...group,
          membersEmail: [], // Enviamos el grupo sin correos electrónicos en la creación
        });
        // Luego actualizamos el grupo con los correos seleccionados
        await userAuthorizationProxy.updateGroup(createdGroup.id as string, {
          ...createdGroup,
          membersEmail: group.membersEmail,
        });
